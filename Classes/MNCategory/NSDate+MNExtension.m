...
@implementation NSDate (MNExtension)
...
+ (NSString *)timeWithTimeIntervalString:(float )sinetime
{
    // 格式化时间
    ...
    // 毫秒值转化为秒
    NSDate* date = [NSDate dateWithTimeIntervalSince1970: sinetime/ 1000.0];
...
{
    NSDate *firstDay = nil;
    if ([[NSCalendar currentCalendar] rangeOfUnit:NSCalendarUnitWeekday startDate:&firstDay interval:NULL forDate:self]) {
        return firstDay;
    }
...
- (NSDate *)yc_firstDayOfTheWeekInTheMonth
{
    ...
    if ([[NSCalendar currentCalendar] rangeOfUnit:NSCalendarUnitWeekday startDate:&firstDayOfTheWeek interval:NULL forDate:self]) {
...
- (NSString *)yc_yearMonthString
{
    ...
    return [NSString stringWithFormat:@"%ld年%ld月",(long)components.year,(long)components.month];
}
#pragma mark - 月日
...
- (NSString *)stringOfDateWithFormat:(MNFormatterStyle)style
{
    return [[[MNDateFormatter shareFormatter] defaultDateFormatterWithStyle:style] stringFromDate:self];
}

//style  应该和 dateString的格式是一一对应的
+(NSDate*)dateOfStringWithFormat:(MNFormatterStyle)style WithDateString:(NSString *)dateString{
    
    
    return  [[[MNDateFormatter shareFormatter] defaultDateFormatterWithStyle:style] dateFromString:dateString];
